...
class AdminController{
        ...
        try{
            await Course.find()
                .then(function (courses) {
                    res.render('admin/adminCourse',{courses:multipleMongooseToObject(courses)})
                })
    ...
    async adminArticle(req, res){
        try{
            await News.find()
                .then(function(news){
                    res.render('admin/adminArticle',{news:multipleMongooseToObject(news)})
            })
        }catch (e) {
            console.log(e);
            res.status(401).json(e)
    ...
    async adminUser(req,res){
        try{
            await User.find()
                .then(function (users) {
                   res.render('admin/adminUser',{users:multipleMongooseToObject(users)})
                })
